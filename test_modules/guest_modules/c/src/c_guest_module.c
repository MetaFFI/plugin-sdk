...
const char* c_guest_join_strings(CGuestStringArray arr)
{
	...
	for (size_t i = 0; i < arr.len; ++i)
	{
		metaffi_strcat(out, total, arr.data[i]);
		if (i + 1 < arr.len)
		{
			metaffi_strcat(out, total, ",");
		}
	}
	return out;
}
...
void c_guest_testmap_set(CGuestTestMap* map, const char* key, CGuestAny value)
{
	...
	CGuestStringIntPair* new_items = (CGuestStringIntPair*)realloc(
		map->items, sizeof(CGuestStringIntPair) * (map->len + 1));
	if (!new_items)
	{
		return;
	}
	map->items = new_items;
	map->items[map->len].key = metaffi_strdup(key);
...
void c_guest_testmap_set_name(CGuestTestMap* map, const char* name)
{
	...
	free(map->name);
	map->name = name ? metaffi_strdup(name) : NULL;
}
...
CGuestStringIntMap c_guest_make_string_int_map(void)
{
	...
	map.items[0].key = metaffi_strdup("a");
	map.items[0].value = 1;
	map.items[1].key = metaffi_strdup("b");
	map.items[1].value = 2;
	return map;
}
...
const char* c_guest_join_variadic(const char* prefix, int count, ...)
{
	...
	metaffi_strcpy(out, total, prefix);
	va_start(args, count);
	for (int i = 0; i < count; ++i)
	{
		const char* val = va_arg(args, const char*);
		metaffi_strcat(out, total, ":");
		metaffi_strcat(out, total, val);
	}
	va_end(args);
	return out;
}
