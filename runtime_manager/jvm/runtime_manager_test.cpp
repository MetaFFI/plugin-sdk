...
struct test_env
{
	explicit test_env(jvm_runtime_manager& manager)
...
constexpr const char* test_class_name = "com.metaffi.jvm.TestModule";
constexpr const char* test_inner_class_name = "com.metaffi.jvm.TestModule.Inner";
...
static jvm_installed_info get_test_jvm_info()
{
	...
	METAFFI_DEBUG(LOG, "jvm_runtime_manager_test: Detected JVMs:");
	...
	for(const auto& preferred : preferred_versions)
	{
		for(const auto& info : jvms)
		{
			...
			{
				METAFFI_DEBUG(LOG, "jvm_runtime_manager_test: Selected JVM: {}", info.home);
				return info;
			}
		}
	}

	METAFFI_DEBUG(LOG, "jvm_runtime_manager_test: Selected JVM: {}", jvms.front().home);
	return jvms.front();
}
...
TEST_SUITE("JDK Runtime Manager Tests")
{
	...
	{// TODO: NO reason to "load_runtime", as get_test_jvm_info() loads all available...
		...
		jvm_runtime_manager manager(info);
	...
	{
		jvm_runtime_manager manager(get_test_jvm_info());
	...
	TEST_CASE("4.2 Load Invalid Module Path")
	{
		...
		CHECK(expect_no_throw([&]() { manager.load_runtime(); }));
		CHECK(expect_throw([&]() {
			(void)manager.load_module("C:/invalid/path/jvm_runtime_manager_missing.jar");
		}));
	}
