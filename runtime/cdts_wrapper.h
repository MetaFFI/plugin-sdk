...
#define on_parse_string(type) \
    ...
    std::function<void(void* values_to_set, int index, const type* parray_to_set, const metaffi_size* pelements_lengths_to_set, const metaffi_size* parray_dimensions_lengths, const metaffi_size& array_dimensions)>  on_##type##_array;
		...
		std::function<void(void* values_to_set, int index, const cdt_metaffi_handle& val_to_set)> on_metaffi_handle;
		std::function<void(void* values_to_set, int index, const cdt_metaffi_handle* parray_to_set,
		                   const metaffi_size* parray_dimensions_lengths,
		                   const metaffi_size& array_dimensions)> on_metaffi_handle_array;
	...
	on_##type##_array(std::move(on_##type##_array))
		...
		cdts_parse_callbacks
				(
						...
						decltype(on_metaffi_handle) on_metaffi_handle,
				   ...
				   on_metaffi_handle(std::move(on_metaffi_handle)),
...
#define on_build_string_interface(type) \
    ...
    virtual void set_##type##_array(void* values_to_set, int index, type*& parray_to_set, metaffi_size*& pelements_lengths_to_set, metaffi_size*& parray_dimensions_lengths, metaffi_size& array_dimensions, metaffi_bool& is_free_required, int starting_index) = 0;
		...
		virtual void set_metaffi_handle(void *values_to_set, int index, cdt_metaffi_handle &val_to_set, int starting_index) = 0;
		virtual void set_metaffi_handle_array(void *values_to_set, int index, cdt_metaffi_handle *&parray_to_set,
		                                          metaffi_size *&parray_dimensions_lengths, metaffi_size &array_dimensions,
		                                          metaffi_bool &is_free_required, int starting_index) = 0;
...
#define set_build_string(type) \
    ...
    std::function<void(void* values_to_set, int index, type*& parray_to_set, metaffi_size*& pelements_lengths_to_set, metaffi_size*& parray_dimensions_lengths, metaffi_size& array_dimensions, metaffi_bool& is_free_required, int starting_index)>  set_##type##_array;
		...
		std::function<void(void *values_to_set, int index, cdt_metaffi_handle& val_to_set, int starting_index)> set_metaffi_handle;
		std::function<void(void *values_to_set, int index, cdt_metaffi_handle* parray_to_set,
		                   metaffi_size *&parray_dimensions_lengths, metaffi_size& array_dimensions,
		                   metaffi_bool &is_free_required, int starting_index)> set_metaffi_handle_array;
	...
	set_##type##_array(std::move(set_##type##_array))
		...
		cdts_build_callbacks
				(
				   ...
				   set_metaffi_handle(std::move(set_metaffi_handle)),
				   set_metaffi_handle_array(std::move(set_metaffi_handle_array)),
...
*************************************************/
	...
	public:
		...
		void set(int index, metaffi_handle* v, int length) const;
			...
			switch (cur_type)
			{
				...
				case metaffi_handle_type | metaffi_array_type:
				{
					callbacks.on_metaffi_handle_array(values_to_set, index,
					                                      this->cdts[index].cdt_val.metaffi_handle_array_val.vals,
					                                      this->cdts[index].cdt_val.metaffi_handle_array_val.dimensions_lengths,
		...
		for(int index=0 ; index<types_length ; index++)
		{
			...
			switch(cur_type)
			{
				...
				case metaffi_handle_type:
				{
					...
					callbacks.set_metaffi_handle(values_to_set, index, val, starting_index);
					this->cdts[index].type = metaffi_handle_type;
					this->cdts[index].cdt_val.metaffi_handle_val.val = val.val;
				...
				case metaffi_handle_type | metaffi_array_type:
				{
					...
					metaffi_size *dimensions_lengths;
					metaffi_bool free_required;
					callbacks.set_metaffi_handle_array(values_to_set, index, array, dimensions_lengths, dimensions, free_required, starting_index);
					this->cdts[index].type = metaffi_handle_array_type;
					this->cdts[index].free_required = free_required;
					this->cdts[index].cdt_val.metaffi_handle_array_val.vals = array;
